class IOCallbacksStorage(object):
    ...
    def on_output(self, value):
        """
        This function is called whenever the AsciiMarbles program returns a value
        Note that the value may be a string
        """
        raise Exception('on_output: not implemented')

    def on_finish(self):
        """ This function is called when the AsciiMarbles program finished executing """
        raise Exception('on_finish: not implemented')

    def on_error(self, error_text):
        """ This function is pretty self explanatory; it catches errors with the AsciiMarble program that occur during interpretetion """
